...
describe('og helpers', () => {
  ...
  it('builds metadata with owner and summary', () => {
    ...
    expect(meta.title).toBe('Weather — OnionHub')
  ...
  it('uses defaults when owner and summary are missing', () => {
    ...
    expect(meta.title).toBe('parser — OnionHub')
    expect(meta.description).toMatch(/OnionHub — a fast skill registry/i)
  ...
  it('fetches skill metadata when response is ok', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/v1/skills/weather', {
  ...
  it('fetches soul metadata when response is ok', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/v1/souls/north-star', {
      headers: { Accept: 'application/json' },
    })
  })

  it('uses explicit api origin when provided', async () => {
    const fetchMock = vi.fn(async () => ({
      ok: true,
      json: async () => ({
        skill: { displayName: 'Weather', summary: 'Forecasts' },
    ...
    await fetchSkillMeta('weather', 'https://registry.example.com')

    expect(fetchMock).toHaveBeenCalledWith('https://registry.example.com/api/v1/skills/weather', {
      headers: { Accept: 'application/json' },
    })
  })
