...
- Browser login opens the configured site `/cli/auth` page and completes via a loopback callback.
- Token stored in `~/Library/Application Support/clawhub/config.json` on macOS (override via `CLAWHUB_CONFIG_PATH`).
...
- Site: resolved from `--site` or `CLAWHUB_SITE`
- Registry: discovered from `/.well-known/clawhub.json` on the site (override via `--registry` or `CLAWHUB_REGISTRY`)
