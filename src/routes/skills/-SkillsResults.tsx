...
import { getSkillBadges } from '../../components/skillDetailUtils'
import { skillsCopy } from '../../copy/skills'
import { getPlatformLabels } from '../../lib/badges'
  ...
  return (
    <>
      {isLoadingSkills ? (
        ...
        <div className="card">
          {paginationStatus === 'Exhausted' || hasQuery ? copy.empty : copy.loading}
        </div>
      ) : view === 'cards' ? (
        <div className="grid">
          {sorted.map((entry) => {
            ...
            return (
              <SkillCard
                ...
                chip={isPlugin ? copy.pluginBundle : undefined}
                platformLabels={platforms.length ? platforms : undefined}
                summaryFallback={copy.cardSummaryFallback}
                meta={
                  <div className="skill-card-footer-rows">
                    <UserBadge user={entry.owner} fallbackHandle={ownerHandle} link={false} />
                    <div className="stat">
              ...
              <Link key={skill._id} className="skills-row" to={skillHref}>
                <div className="skills-row-main">
                  <div className="skills-row-title">
                    ...
                    {isPlugin ? <span className="tag tag-accent tag-compact">{copy.pluginBundle}</span> : null}
                    {platforms.map((label) => (
                      <span key={label} className="tag tag-compact">{label}</span>
                    ))}
                  </div>
                  <div className="skills-row-summary">{skill.summary ?? copy.listSummaryFallback}</div>
      ...
      {canLoadMore || isLoadingMore ? (
        ...
        >
          {canAutoLoad ? (
            isLoadingMore ? copy.loadingMore : copy.scrollToLoadMore
          ) : (
