...
export async function listTextFiles(root: string) {
  ...
  const ig = ignore()
  ig.add(['.git/', 'node_modules/', ...brand.workspaceReadDirs.map((dir) => `${dir}/`)])
  await addIgnoreFile(ig, join(absRoot, '.gitignore'))
  for (const fileName of brand.ignoreReadFiles) {
    await addIgnoreFile(ig, join(absRoot, fileName))
  }
...
export async function readLockfile(workdir: string): Promise<Lockfile> {
  const paths = getCliBrand().workspaceReadDirs.map((dir) => join(workdir, dir, 'lock.json'))
...
export async function writeLockfile(workdir: string, lock: Lockfile) {
  const path = join(workdir, getCliBrand().workspaceDirName, 'lock.json')
  await mkdir(dirname(path), { recursive: true })
  await writeFile(path, `${JSON.stringify(lock, null, 2)}\n`, 'utf8')
}

export async function readSkillOrigin(skillFolder: string): Promise<SkillOrigin | null> {
  const paths = getCliBrand().workspaceReadDirs.map((dir) => join(skillFolder, dir, 'origin.json'))
  for (const path of paths) {
    try {
      const raw = await readFile(path, 'utf8')
...
export async function writeSkillOrigin(skillFolder: string, origin: SkillOrigin) {
  const path = join(skillFolder, getCliBrand().workspaceDirName, 'origin.json')
  await mkdir(dirname(path), { recursive: true })
