...
import { getEnvCandidates } from '../runtime.js'
...
export function getCliCommit() {
  const candidates = [...getEnvCandidates('COMMIT').map((key) => process.env[key]), process.env.VERCEL_GIT_COMMIT_SHA, process.env.GITHUB_SHA, process.env.COMMIT_SHA]
