...
import { formatInstallCommand, formatInstallLabel } from '../lib/badges'
...
export function SkillInstallCard({ clawdis, osLabels }: SkillInstallCardProps) {
  const copy = skillsCopy.detail.installCard
        ...
        {hasRuntimeRequirements ? (
          <div className="skill-panel">
            ...
            <div className="skill-panel-body">
              ...
              {clawdis?.primaryEnv ? (
                <div className="stat">
                  <strong>{copy.primaryEnv}</strong>
                  <span>{clawdis.primaryEnv}</span>
                </div>
              ) : null}
              {envVars.length > 0 ? (
                <div className="stat">
                  <strong>{copy.environmentVariables}</strong>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', marginTop: '0.25rem' }}>
                    {envVars.map((env, index) => (
                      <div key={`${env.name}-${index}`} style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
                        ...
                        {env.required === false ? (
                          <span style={{ color: 'var(--ink-soft)', fontSize: '0.75rem' }}>{copy.optional}</span>
                        ) : env.required === true ? (
                          <span style={{ color: 'var(--ink-accent)', fontSize: '0.75rem' }}>{copy.required}</span>
                        ) : null}
                        {env.description ? (
                ...
                <div key={`${dep.name}-${index}`} className="stat">
                  <div>
                    ...
                    {dep.repository && dep.repository !== dep.url ? (
                      <div style={{ fontSize: '0.8rem' }}>
                        <a href={dep.repository} target="_blank" rel="noopener noreferrer">{copy.source}</a>
                      </div>
                    ) : null}
                  </div>
                </div>
