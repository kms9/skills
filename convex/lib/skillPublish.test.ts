...
describe('skillPublish', () => {
  ...
  it('merges gitlab source into metadata', () => {
    const merged = __test.mergeSourceIntoMetadata(undefined, {
      kind: 'gitlab',
      url: 'https://gitlab.com/a/b',
      repo: 'a/b',
      ref: 'main',
      commit: 'fedcba9876543210012345678901234567890123',
      path: 'skills/demo',
      importedAt: 456,
    })

    expect((merged as Record<string, unknown>).source).toEqual(
      expect.objectContaining({
        kind: 'gitlab',
        repo: 'a/b',
        ref: 'main',
