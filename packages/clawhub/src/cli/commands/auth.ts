...
import { readGlobalConfig, writeAuthBinding, writeGlobalConfig } from '../../config.js'
...
export async function cmdLoginFlow(
  ...
  const authBase = discovery?.authBase?.trim() || opts.authBase?.trim() || opts.site
...
export async function cmdLogin(
  ...
  const token = tokenFlag || (await promptHidden(`${getCliBrand().displayName} token: `))
...
export async function cmdLogout(opts: GlobalOpts) {
  const cfg = await readGlobalConfig()
  const registry = (await getRegistry(opts, { cache: true })) || opts.registry
  await writeGlobalConfig(
    writeAuthBinding(cfg, {
      envName: opts.envName,
      site: opts.site,
...
export async function cmdWhoami(opts: GlobalOpts) {
  const token = await requireAuthToken(opts)
  const registry = await getRegistry(opts, { cache: true })
