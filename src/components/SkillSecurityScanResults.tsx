...
function getScanStatusInfo(status: string) {
  const copy = skillsCopy.detail.security
  switch (status.toLowerCase()) {
    case 'benign':
    case 'clean':
      return { label: copy.benign, className: 'scan-status-clean' }
    case 'malicious':
      return { label: copy.malicious, className: 'scan-status-malicious' }
    case 'suspicious':
      return { label: copy.suspicious, className: 'scan-status-suspicious' }
    case 'loading':
      return { label: copy.loading, className: 'scan-status-pending' }
    case 'pending':
    case 'not_found':
      return { label: copy.pending, className: 'scan-status-pending' }
    case 'error':
    case 'failed':
      return { label: copy.error, className: 'scan-status-error' }
    default:
    ...
    <div className={`analysis-detail${isOpen ? ' is-open' : ''}`}>
      <button
        ...
        <span className="analysis-detail-toggle">
          {copy.details} <span className="chevron">{'\u25BE'}</span>
        </span>
      </button>
      <div className="analysis-body">
        ...
        {analysis.findings ? (
          <div className="scan-findings-section">
            <div className="scan-findings-title">{copy.findingsInContext}</div>
            {(() => {
...
export function SecurityScanResults({
  ...
  return (
    <div className="scan-results-panel">
      <div className="scan-results-title">{copy.title}</div>
      <div className="scan-results-list">
        ...
        {isCodeInsight && aiAnalysis && (vtStatus === 'malicious' || vtStatus === 'suspicious') ? (
          <div className={`code-insight-analysis ${vtStatus}`}>
            <div className="code-insight-label">{copy.codeInsight}</div>
        ...
        {llmStatusInfo && llmAnalysis ? (
          <div className="scan-result-row">
            ...
            {llmAnalysis.confidence ? (
              <span className="scan-result-confidence">{llmAnalysis.confidence} {copy.confidence}</span>
            ) : null}
          </div>
        ) : null}
        {llmAnalysis &&
